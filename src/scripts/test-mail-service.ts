...
async function testMailService() {
  ...
  try {
    ...
    if (!brevoApiKey) {
      ...
      const maskedKey = brevoApiKey.length > 10 
      ...
      if (brevoApiKey.startsWith('xkeysib-')) {
        ...
        logger.warn('⚠️ API Key format may be incorrect (should start with xkeysib-)');
      }
    }
    ...
    logger.error(`❌ Error during mail service test: ${error.message}`, error.stack);
  }
}
